...
function AiAssistants() {
  ...
  const handleOnSelect = (assistant: AssistantType) => {
    const exists = selectedAssistant.find(item => item.id === assistant.id);
    if (exists) {
  ...
  const handleOnClickContinue = async () => {
    ...
    try {
      ...
      router.push('/workspace'); // Redirect **only after clicking Continue**
    ...
    <div className='px-10 mt-15 md:px-28 lg:px-36 xl:px-48'>
      ...
      <div className='grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-5 mt-5'>
        {AiAssistantsList.map((assistant, index) => (
          <BlurFade key={assistant.image} delay={0.1 + index * 0.15} inView>
            <div
            ...
            >
              ...
              <h2 className='dark:text-gray-300 text-md font-light dark:font-thin text-gray-500'>
                {assistant.title}
              </h2>
            </div>
          </BlurFade>
